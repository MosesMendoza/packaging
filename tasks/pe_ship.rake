  ...
  namespace :pe do
    ...
    task :ship_debs => "pl:fetch" do
      ...
      retry_on_fail(:times => 3) do
        cd "pkg/pe/deb" do
          Dir["**/*.deb"].each do |deb|
            rsync_to(deb, Pkg::Config.apt_host, "#{target_path}/#{File.dirname(deb)}/")
          end
        end
      end
    ...
    namespace :remote do
      ...
      task :update_yum_repo => "pl:fetch" do
        remote_ssh_cmd(Pkg::Config.yum_host, "for dir in  $(find #{Pkg::Config.apt_repo_path}/#{Pkg::Config.pe_version}/repos/{sles,el}* -type d | grep -v repodata | grep -v cache | xargs)  ; do pushd $dir; sudo createrepo -q -d --update .; popd &> /dev/null ; done; sync")
      end
      ...
      task :apt, :incoming do |t, args|
        ...
        remote_ssh_cmd(Pkg::Config.apt_host, "/usr/bin/repsimple add_all \
            --confdir /etc/reprepro/#{Pkg::Config.pe_version} \
            --basedir #{Pkg::Config.apt_repo_path}/#{Pkg::Config.pe_version}/repos/debian \
            --databasedir /var/lib/reprepro/#{Pkg::Config.pe_version} \
